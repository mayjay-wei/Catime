...
static void EnsureCriticalSectionInitialized(void) {
    if (InterlockedCompareExchange(&g_IniCriticalSectionInitialized,
                                   INI_CS_INITIALIZING, INI_CS_UNINITIALIZED) ==
        INI_CS_UNINITIALIZED) {
...
static HANDLE GetConfigWriteMutex(void) {
    if (g_ConfigWriteMutex == NULL) {
        g_ConfigWriteMutex =
            CreateMutexW(NULL, FALSE, L"CatimeConfigWriteMutex");
    }
...
static IniEntry* CreateEntry(IniSection* section, const char* key,
                             const char* value) {
    if (!section || !key)
        return NULL;
...
static BOOL WriteIniToFile(IniFile* ini, const char* filePath) {
    ...
    for (IniSection* section = ini->sections; section;
 ...
 */
static const char* GetIniValue(const char* section, const char* key,
                               const char* filePath) {
 ...
 */
static BOOL SetIniValue(const char* section, const char* key, const char* value,
                        const char* filePath) {
    IniFile* ini = EnsureIniLoaded(filePath);
    if (!ini)
        return FALSE;
 ...
 */
BOOL FileExists(const char* filePath) { return FileExistsUtf8(filePath); }
...
BOOL ReadIniBool(const char* section, const char* key, BOOL defaultValue,
    ...
    if (value && *value) {
        result = (StrEqualNoCase(value, "TRUE") || StrEqualNoCase(value, "1") ||
...
BOOL WriteIniMultipleAtomic(const char* filePath, const IniKeyValue* updates,
    ...
    for (size_t i = 0; i < count; i++) {
        if (updates[i].section && updates[i].key && updates[i].value) {
            SetIniValue(updates[i].section, updates[i].key, updates[i].value,
                        filePath);
        }
    }
