...
int32_t current_pomodoro_time_index = 0;
POMODORO_PHASE current_pomodoro_phase = POMODORO_PHASE_IDLE;
int32_t complete_pomodoro_cycles = 0;
static int32_t pomodoro_initial_times_count = 0;
static int32_t pomodoro_initial_loop_count = 0;
static int32_t pomodoro_initial_times[MAX_POMODORO_TIMES] = {};
static DWORD last_timer_tick = 0;
static int32_t ms_accumulator = 0;
...
static wchar_t* SafeUtf8ToWide(const char* utf8String, wchar_t* buffer,
                               size_t bufferSize) {
    if (!utf8String || !buffer || utf8String[0] == '\0') {
        return nullptr;
    }

    return Utf8ToWide(utf8String, buffer, bufferSize) ? buffer : nullptr;
}

static void ShowTimeoutNotification(HWND hwnd, const char* messageUtf8,
                                    BOOL playSound) {
    ...
    const wchar_t* messageW =
        SafeUtf8ToWide(messageUtf8, messageBuffer, MESSAGE_BUFFER_SIZE);
...
static bool ExecuteSystemAction(HWND hwnd, TimeoutActionType action) {
    for (size_t i = 0; i < sizeof(SYSTEM_ACTIONS) / sizeof(SYSTEM_ACTIONS[0]);
        ...
        const int32_t result = system(SYSTEM_ACTIONS[i].command);
        if (result != 0) {
            LOG_WARNING(
                "System action '%s' failed (code: %d). May need "
...
static bool HandleRetryTimer(HWND hwnd, UINT timerId, int32_t* retryCount,
                             RetrySetupCallback callback) {
    if (*retryCount == 0) {
...
static void HandleTimeoutActions(HWND hwnd) {
    switch (CLOCK_TIMEOUT_ACTION) {
        ...
        case TIMEOUT_ACTION_OPEN_FILE: {
            ...
            if (MultiByteToWideChar(CP_UTF8, 0, CLOCK_TIMEOUT_FILE_PATH, -1,
                                    wPath, MAX_PATH) <= 0) {
            ...
            if ((uintptr_t)result <= 32) {
                LOG_WARNING("Failed to open timeout file: %s (error: %u)",
                            CLOCK_TIMEOUT_FILE_PATH, (uintptr_t)result);
            }
            break;
        }
        ...
        case TIMEOUT_ACTION_OPEN_WEBSITE: {
            ...
            HINSTANCE result =
                ShellExecuteW(NULL, L"open", wUrl, NULL, NULL, SW_NORMAL);
        ...
        case TIMEOUT_ACTION_SLEEP:
            /* These system actions are handled earlier by
             * ExecuteSystemAction(). */
            break;
    }
}

static void FormatPomodoroTime(int seconds, wchar_t* buffer,
                               size_t bufferSize) {
    if (seconds >= 60) {
        int minutes = seconds / 60;
        int remaining_seconds = seconds % 60;
        if (remaining_seconds > 0) {
            _snwprintf_s(buffer, bufferSize, _TRUNCATE, L"%dm%ds", minutes,
                         remaining_seconds);
        } else {
...
static bool HandlePomodoroCompletion(HWND hwnd) {
    ...
    int32_t completedIndex = current_pomodoro_time_index;
    const int32_t times_count =
        (pomodoro_initial_times_count <= 0) ? 1 : pomodoro_initial_times_count;
    const int32_t loop_count =
        (pomodoro_initial_loop_count <= 0) ? 1 : pomodoro_initial_loop_count;

    /* Current step within this cycle (1-based) */
    int32_t stepInCycle = completedIndex + 1;
    /* Current cycle number (1-based) */
    int32_t currentCycle = complete_pomodoro_cycles + 1;

    if (completedIndex < pomodoro_initial_times_count) {
        FormatPomodoroTime(pomodoro_initial_times[completedIndex], timeStr,
    ...
    if (!AdvancePomodoroState()) {
        ...
        if (times_count > 1 || loop_count > 1) {
            _snwprintf_s(completionMsg, sizeof(completionMsg) / sizeof(wchar_t),
                         _TRUNCATE, L"%ls %ls (%ls%d/%d%ls %d/%d)", timeStr,
                         completed_text, cycle_text, currentCycle, loop_count,
                         round_text, stepInCycle, times_count);
        } else {
            _snwprintf_s(completionMsg, sizeof(completionMsg) / sizeof(wchar_t),
                         _TRUNCATE, L"%ls %ls", timeStr, completed_text);
        }
        ...
        const wchar_t* cycle_complete_text =
    ...
    const int32_t next_duration_sec =
        pomodoro_initial_times[current_pomodoro_time_index];
...
static void HandleCountdownCompletion(HWND hwnd) {
    ...
    const char* actionNames[] = {"MESSAGE",  "LOCK",      "SHUTDOWN",
                                 "RESTART",  "SLEEP",     "SHOW_TIME",
                                 "COUNT_UP", "OPEN_FILE", "OPEN_WEBSITE"};
    const char* actionName =
        (CLOCK_TIMEOUT_ACTION >= 0 && CLOCK_TIMEOUT_ACTION < 9)
            ? actionNames[CLOCK_TIMEOUT_ACTION]
            : "UNKNOWN";
...
static bool HandleMainTimer(HWND hwnd) {
    ...
    if (CLOCK_HOLOGRAPHIC_EFFECT) {
        ...
        const int32_t pixels = rect.right * rect.bottom;
        const DWORD minInterval = (pixels < 30000)    ? 0
                                  : (pixels < 100000) ? 50
                                  : (pixels < 300000) ? 100
                                                      : 150;
        if (minInterval > 0 && (now_tick - s_lastRenderTime) < minInterval)
            shouldRender = false;
    } else if (!g_AppConfig.display.time_format.show_milliseconds &&
               !CLOCK_SHOW_CURRENT_TIME) {
        if (s_lastRenderTime != 0 && (now_tick - s_lastRenderTime) < 250)
            shouldRender = false;
    }
    ...
    int32_t current_elapsed_sec = 0;

    if (CLOCK_COUNT_UP) {
        const int64_t elapsed_ms = (current_time_ms >= g_start_time)
                                       ? current_time_ms - g_start_time
                                       : 0;
        current_elapsed_sec = (int32_t)(elapsed_ms / 1000);
        countup_elapsed_time = current_elapsed_sec;
    } else {
        const int64_t remaining_ms = (g_target_end_time >= current_time_ms)
                                         ? g_target_end_time - current_time_ms
                                         : 0;

        /* Use ceiling division to prevent "00:00" display while time remains
         * (e.g. 0.9s -> 1s) */
        const int32_t remaining_sec_rounded =
            (int32_t)((remaining_ms + 999) / 1000);

        current_elapsed_sec = (CLOCK_TOTAL_TIME < remaining_sec_rounded)
                                  ? 0
                                  : CLOCK_TOTAL_TIME - remaining_sec_rounded;
    ...
    if (!countdown_message_shown) {
        ...
        if (IsActivePomodoroTimer()) {
            if (HandlePomodoroCompletion(hwnd)) {
                return true;
            }
        } else {
...
void InitializePomodoro(void) {
    ...
    memset(pomodoro_initial_times, 0, sizeof(pomodoro_initial_times));
    int32_t copy_count = (pomodoro_initial_times_count < MAX_POMODORO_TIMES)
...
BOOL HandleTimerEvent(HWND hwnd, WPARAM wp) {
    static int32_t topmost_retry = 0;
    static int32_t visibility_retry = 0;

    switch (wp) {
        case TIMER_ID_TOPMOST_RETRY:
            return HandleRetryTimer(hwnd, TIMER_ID_TOPMOST_RETRY,
                                    &topmost_retry, SetupTopmostWindow);

        case TIMER_ID_VISIBILITY_RETRY:
            return HandleRetryTimer(hwnd, TIMER_ID_VISIBILITY_RETRY,
                                    &visibility_retry, SetupVisibilityWindow);
