...
void LoadPomodoroConfig(void) {
    ...
    g_AppConfig.pomodoro.loop_count =
        ReadIniInt(INI_SECTION_POMODORO, "POMODORO_LOOP_COUNT", 1, configPath);
    if (g_AppConfig.pomodoro.loop_count < 1)
...
void BuildPomodoroMenu(HMENU hMenu) {
    ...
    for (int i = 0; i < g_AppConfig.pomodoro.times_count; i++) {
        FormatPomodoroTime(g_AppConfig.pomodoro.times[i], timeBuffer,
                           sizeof(timeBuffer) / sizeof(wchar_t));

        UINT menuId;
        if (i == 0)
            menuId = CLOCK_IDM_POMODORO_WORK;
        else if (i == 1)
            menuId = CLOCK_IDM_POMODORO_BREAK;
        else if (i == 2)
            menuId = CLOCK_IDM_POMODORO_LBREAK;
        else
            menuId = CLOCK_IDM_POMODORO_TIME_BASE + i;

        BOOL isCurrentPhase =
            (current_pomodoro_phase != POMODORO_PHASE_IDLE &&
             current_pomodoro_time_index == i && !CLOCK_SHOW_CURRENT_TIME &&
             !CLOCK_COUNT_UP &&
             CLOCK_TOTAL_TIME == g_AppConfig.pomodoro.times[i]);

        AppendMenuW(hPomodoroMenu,
                    MF_STRING | (isCurrentPhase ? MF_CHECKED : MF_UNCHECKED),
                    menuId, timeBuffer);
    }
    ...
    _snwprintf_s(menuText, _countof(menuText), _TRUNCATE,
                 ...
                 g_AppConfig.pomodoro.loop_count);
    AppendMenuW(hPomodoroMenu, MF_STRING, CLOCK_IDM_POMODORO_LOOP_COUNT,
                menuText);
