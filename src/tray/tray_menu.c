...
void ShowColorMenu(HWND hwnd) {
    ...
    /* Edit mode toggle */
    AppendMenuW(hMenu,
                MF_STRING | (CLOCK_EDIT_MODE ? MF_CHECKED : MF_UNCHECKED),
                CLOCK_IDC_EDIT_MODE, GetLocalizedString(NULL, L"Edit Mode"));
    ...
    TrackPopupMenu(hMenu, TPM_LEFTALIGN | TPM_RIGHTBUTTON, pt.x, pt.y, 0, hwnd,
                   NULL);
...
void ShowContextMenu(HWND hwnd) {
    ...
    BOOL timerRunning =
        (!CLOCK_SHOW_CURRENT_TIME &&
         (CLOCK_COUNT_UP || (!CLOCK_COUNT_UP && CLOCK_TOTAL_TIME > 0 &&
    ...
    AppendMenuW(hTimerManageMenu,
                MF_STRING | (timerRunning ? MF_ENABLED : MF_GRAYED),
                CLOCK_IDM_TIMER_PAUSE_RESUME, pauseResumeText);

    BOOL canRestart =
        (!CLOCK_SHOW_CURRENT_TIME && (CLOCK_COUNT_UP || CLOCK_TOTAL_TIME > 0));

    AppendMenuW(
        hTimerManageMenu, MF_STRING | (canRestart ? MF_ENABLED : MF_GRAYED),
        CLOCK_IDM_TIMER_RESTART, GetLocalizedString(NULL, L"Start Over"));

    const wchar_t* visibilityText =
        IsWindowVisible(hwnd) ? GetLocalizedString(NULL, L"Hide Window")
                              : GetLocalizedString(NULL, L"Show Window");

    AppendMenuW(hTimerManageMenu, MF_STRING, CLOCK_IDC_TOGGLE_VISIBILITY,
                visibilityText);
    ...
    AppendMenuW(
        hTimeMenu,
        MF_STRING | (CLOCK_SHOW_CURRENT_TIME ? MF_CHECKED : MF_UNCHECKED),
        CLOCK_IDM_SHOW_CURRENT_TIME,
        GetLocalizedString(NULL, L"Show Current Time"));

    AppendMenuW(
        hTimeMenu, MF_STRING | (CLOCK_USE_24HOUR ? MF_CHECKED : MF_UNCHECKED),
        CLOCK_IDM_24HOUR_FORMAT, GetLocalizedString(NULL, L"24-Hour Format"));

    AppendMenuW(
        hTimeMenu, MF_STRING | (CLOCK_SHOW_SECONDS ? MF_CHECKED : MF_UNCHECKED),
        CLOCK_IDM_SHOW_SECONDS, GetLocalizedString(NULL, L"Show Seconds"));
    ...
    for (int i = 0; i < time_options_count; i++) {
        ...
        FormatPomodoroTime(time_options[i], menu_item,
                           sizeof(menu_item) / sizeof(wchar_t));
