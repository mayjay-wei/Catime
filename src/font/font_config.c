...
extern BOOL WriteIniString(const char* section, const char* key,
...
void WriteConfigFont(const char* fontFileName, BOOL shouldReload) {
     ...
     * file with the same name in a subdirectory, causing incorrect path
     * storage.
     *
     * The fontFileName parameter is already a correct relative path from the
     * cache, selected by the user from the menu. We just need to add the config
     * prefix.
     */
