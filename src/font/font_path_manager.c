...
BOOL GetFontsFolderW(wchar_t* outW, size_t size, BOOL ensureCreate) {
    ...
    if (!Utf8ToWide(configPathUtf8, configPathW, MAX_PATH))
        return FALSE;
    ...
    if (wcslen(outW) + 1 + wcslen(L"resources\\fonts") + 1 >= size)
        return FALSE;
...
BOOL BuildFullFontPath(const char* relativePath, char* outAbsolutePath,
                       size_t bufferSize) {
    if (!relativePath || !outAbsolutePath || bufferSize == 0)
        return FALSE;

    /* Check if path is already absolute (contains drive letter or starts with
     * UNC) */
    if (strchr(relativePath, ':') ||
        (strlen(relativePath) > 2 && relativePath[0] == '\\' &&
         relativePath[1] == '\\')) {
    ...
    if (!GetFontsFolderW(fontsFolderW, MAX_PATH, TRUE))
        return FALSE;

    wchar_t relativeW[MAX_PATH] = {0};
    if (!Utf8ToWide(relativePath, relativeW, MAX_PATH))
        return FALSE;

    wchar_t fullW[MAX_PATH] = {0};
    _snwprintf_s(fullW, MAX_PATH, _TRUNCATE, L"%s\\%s", fontsFolderW,
                 relativeW);
...
BOOL CalculateRelativePath(const char* absolutePath, char* outRelativePath,
                           size_t bufferSize) {
    if (!absolutePath || !outRelativePath || bufferSize == 0)
        return FALSE;
    ...
    if (!WideToUtf8(fontsFolderW, fontsFolderUtf8, MAX_PATH))
        return FALSE;
...
const char* ExtractRelativePath(const char* fullConfigPath) {
    if (!IsFontsFolderPath(fullConfigPath))
        return NULL;
 ...
 */
static BOOL SearchFontRecursiveW(const wchar_t* folderPathW,
                                 const wchar_t* targetFileW,
                                 wchar_t* resultPathW, size_t resultCapacity) {
    if (!folderPathW || !targetFileW || !resultPathW)
        return FALSE;
    ...
    do {
        ...
        _snwprintf_s(fullItemPathW, MAX_PATH, _TRUNCATE, L"%s\\%s", folderPathW,
                     findDataW.cFileName);
...
BOOL FindFontInFontsFolder(const char* fontFileName, char* foundPath,
    ...
    if (!fontFileName || !foundPath || foundPathSize == 0)
        return FALSE;
    ...
    if (!Utf8ToWide(fontFileName, targetFileW, MAX_PATH))
        return FALSE;

    /* Search recursively */
    wchar_t resultPathW[MAX_PATH] = {0};
    if (!SearchFontRecursiveW(fontsFolderW, targetFileW, resultPathW,
                              MAX_PATH)) {
        return FALSE;
    }
...
BOOL AutoFixFontPath(const char* fontFileName, FontPathInfo* pathInfo) {
    ...
    strncpy(pathInfo->fileName, GetFileNameU8(fontFileName),
            sizeof(pathInfo->fileName) - 1);
...
bool CheckAndFixFontPath(void) {
    ...
    GetFontNameFromFile(pathInfo.absolutePath, FONT_INTERNAL_NAME,
                        sizeof(FONT_INTERNAL_NAME));

    /* Write to config */
    char config_path[MAX_PATH];
    GetConfigPath(config_path, MAX_PATH);
    WriteIniString("Display", "FONT_FILE_NAME", pathInfo.configPath,
                   config_path);
