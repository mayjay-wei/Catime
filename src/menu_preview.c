...
extern void WriteConfigEffect(
    EffectType effect); /* To be implemented if needed, or use existing logic */
...
BOOL IsPreviewActive(void) { return g_previewState.type != PREVIEW_TYPE_NONE; }

PreviewType GetActivePreviewType(void) { return g_previewState.type; }
    ...
    switch (type) {
        case PREVIEW_TYPE_COLOR: {
            ...
            strncpy_s(g_previewState.data.colorHex,
                      sizeof(g_previewState.data.colorHex), colorHex,
                      _TRUNCATE);
            if (hwnd)
        ...
        case PREVIEW_TYPE_FONT: {
            ...
            strncpy_s(g_previewState.data.font.fontName, MAX_PATH, fontName,
                      _TRUNCATE);

            HINSTANCE hInstance = GetModuleHandle(NULL);
            LoadFontByNameAndGetRealName(
                hInstance, fontName, g_previewState.data.font.internalName,
        ...
        case PREVIEW_TYPE_ANIMATION: {
            ...
            strncpy_s(g_previewState.data.animationPath, MAX_PATH, animPath,
                      _TRUNCATE);
...
void GetActiveFont(char* outFontName, char* outInternalName,
                   size_t bufferSize) {
    if (!outFontName || !outInternalName || bufferSize == 0)
        return;

    if (g_previewState.type == PREVIEW_TYPE_FONT) {
        strncpy_s(outFontName, bufferSize, g_previewState.data.font.fontName,
                  _TRUNCATE);
        strncpy_s(outInternalName, bufferSize,
                  g_previewState.data.font.internalName, _TRUNCATE);
    } else if (IS_PREVIEWING) {
        /* Support for Drag & Drop live preview */
        strncpy_s(outFontName, bufferSize, PREVIEW_FONT_NAME, _TRUNCATE);
        strncpy_s(outInternalName, bufferSize, PREVIEW_INTERNAL_NAME,
                  _TRUNCATE);
    } else {
...
TimeFormatType GetActiveTimeFormat(void) {
    return (g_previewState.type == PREVIEW_TYPE_TIME_FORMAT)
               ? g_previewState.data.timeFormat
               : g_AppConfig.display.time_format.format;
}
...
EffectType GetActiveEffect(void) {
     ...
     * Silently disable effects when milliseconds are shown, auto-restore when
     * disabled. */
    ...
    if (CLOCK_LIQUID_EFFECT)
        return EFFECT_TYPE_LIQUID;
    if (CLOCK_HOLOGRAPHIC_EFFECT)
        return EFFECT_TYPE_HOLOGRAPHIC;
    if (CLOCK_NEON_EFFECT)
        return EFFECT_TYPE_NEON;
    if (CLOCK_GLASS_EFFECT)
        return EFFECT_TYPE_GLASS;
    if (CLOCK_GLOW_EFFECT)
        return EFFECT_TYPE_GLOW;
...
void ShowWindowForPreview(HWND hwnd) {
    ...
    WriteLog(LOG_LEVEL_INFO,
             "ShowWindowForPreview: visible=%d, showTime=%d, countUp=%d, "
             "total=%d, elapsed=%d, hasContent=%d, didShow=%d",
             isVisible, CLOCK_SHOW_CURRENT_TIME, CLOCK_COUNT_UP,
             CLOCK_TOTAL_TIME, countdown_elapsed_time, hasActiveContent,
    ...
    if (!isVisible || !hasActiveContent) {
        ...
        if (!hasActiveContent) {
            ...
            WriteLog(LOG_LEVEL_INFO,
                     "No active content, creating preview timer: %d",
                     previewTime);
            ...
            WriteLog(LOG_LEVEL_INFO,
                     "Window hidden but has active timer, just showing it");
...
void RestoreWindowVisibility(HWND hwnd) {
    if (!hwnd || !g_previewState.didShowForPreview)
        return;

    WriteLog(
        LOG_LEVEL_INFO,
        "RestoreWindowVisibility: was visible=%d, created preview timer=%d",
    ...
    if (g_previewState.createdPreviewTimer) {
        ...
        WriteLog(LOG_LEVEL_INFO,
                 "Not clearing timer - was showing existing active timer");
    }
...
BOOL GetPreviewTimeText(wchar_t* outText, size_t bufferSize) {
    ...
    int hours =
        CLOCK_USE_24HOUR ? st.wHour : (st.wHour % 12 == 0 ? 12 : st.wHour % 12);
    ...
    if (showMs) {
        ...
        if (format == TIME_FORMAT_FULL_PADDED ||
            format == TIME_FORMAT_ZERO_PADDED) {
        ...
        if (CLOCK_SHOW_SECONDS) {
            if (format == TIME_FORMAT_FULL_PADDED ||
                format == TIME_FORMAT_ZERO_PADDED) {
                _snwprintf_s(outText, bufferSize, _TRUNCATE, L"%02d:%02d:%02d",
                             hours, minutes, seconds);
            } else {
                _snwprintf_s(outText, bufferSize, _TRUNCATE, L"%d:%02d:%02d",
                             hours, minutes, seconds);
            }
        } else {
            if (format == TIME_FORMAT_FULL_PADDED ||
                format == TIME_FORMAT_ZERO_PADDED) {
                _snwprintf_s(outText, bufferSize, _TRUNCATE, L"%02d:%02d",
                             hours, minutes);
            } else {
                _snwprintf_s(outText, bufferSize, _TRUNCATE, L"%d:%02d", hours,
                             minutes);
            }
        }
    }
